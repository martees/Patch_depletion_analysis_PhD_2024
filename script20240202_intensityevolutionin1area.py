...
def intensity_evolution(list_of_images, x_range=None, y_range=None, remove_peaks=True):
    """
    ...
    # Smooth it
    avg_intensities = savitzky_golay(avg_intensities, 41, 1)  # window size 51, polynomial order 3
...
def find_non_visited_sites(silhouettes, dilation_radius, site_size):
    """
    Takes a list of worm silhouettes (time series of matrices, with tracking label of each pixel, 0 is background), a
...
def interactive_intensity_plot_auto_normalization(image_path, start, end, quarter, x_range=None, y_range=None,
                                                  analysis_size=10):
    """
        Function that shows a scrollable plot with on top, an area of the image, and on the bottom, the mean intensity
        evolution for this area. When you scroll, it should switch to the next frame, and display current time on curve.
    ...
    def click_event(event):
        ...
        if event.button is MouseButton.LEFT and event.inaxes == middle_ax:
            ...
            # Look for the best normalization pixels (closest area where the worm never went)
            curr_norm_x, curr_norm_y = find_best_normalization([curr_x - analysis_size, curr_x + analysis_size],
                                                               [curr_y - analysis_size, curr_y + analysis_size])
            # Update the plot of evolution of intensity of the normalization pixel
            normalization_pixel_intensities = intensity_evolution(image_list, [curr_norm_x - analysis_size,
                                                                               curr_norm_x + analysis_size],
                                                                  [curr_norm_y - analysis_size,
                                                                   curr_norm_y + analysis_size])
            normalization_pixel_intensity_plot[0].set_data(range(len(normalization_pixel_intensities)),
                                                           normalization_pixel_intensities)
...
if __name__ == "__main__":
    ...
    silhouettes = np.load(path + "silhouettes.npy")
    ...
    interactive_intensity_plot(path, silhouettes, start_time, end_time, 2, x_limits, y_limits, analysis_size=10)
